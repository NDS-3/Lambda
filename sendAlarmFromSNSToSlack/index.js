	...
	blocks: [
		...
		},
		{
			...
			elements: [
				{
					type: "plain_text",
					text: "발생 시각: " + triggerDate.toLocaleDateString('ko-KR') + ' ' + triggerDate.toLocaleTimeString('ko-KR'),
					emoji: true
				}
			]
		},
...
const sendMessage = async (message) => {
  ...
  return await axios.post(MONITORING_CHANNEL, createMessage(message), { headers });
};

exports.handler = async (event) => {
  event.Records.map(async record => {
    await sendMessage(record.Sns);
  });
